...
for i in range(1,25):
    ...
    for j in range(no_intervals):
        df = pd.read_csv('../output/' + model + '/R_' + model + '/k' + str(i) + '_n25_Info_g' + str(round(gamma_0 + gamma_h * j,4)) + '_b' + str(beta) + '_t' + str(tau) + '_s' + str(sigma) + '_e' + epsilon + '_I' + str(iterMax) + '.csv')
