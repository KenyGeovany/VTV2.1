...
img_name = sys.argv[2]
img_clean = open_img('input/img_deblurring_gaussian/'+img_name+'_small.png', echo=False)
img_noise = open_img('input/img_deblurring_gaussian/'+img_name+'_small_blurred.png', echo=False)
...
# MODELOS
def pdhg_opp_deb_gradient(u, u_0, img_name):
    # RGB to OPP
    u_0 = rgb2opp(u_0, 0.6)
    ...
    # construction kernel
    gaussian_kernel_2d = np.zeros((rows, cols))
    for i in range(rows):
        for j in range(cols):
            if (abs(i - int(rows / 2) <= 12) and abs(j - int(cols / 2) <= 12)):
                gaussian_kernel_2d[i, j] = np.exp(
                    -((i - int(rows / 2)) ** 2 + (j - int(cols / 2)) ** 2) / (2 * (std ** 2)))
    gaussian_kernel_2d = gaussian_kernel_2d / np.sum(gaussian_kernel_2d)
    TF_gaussian_kernel_2d = fft2(fftshift(gaussian_kernel_2d))

    while (mse > tol and k < iterMax) or (k < 50):
        print("k:", k)
        print("mse: ", mse)

        u_kplus1 = np.real(ifft2((fft2(u_0) * TF_gaussian_kernel_2d + (gamma / tau) * fft2(u_k - tau * divergence_2(eta_k))) / (TF_gaussian_kernel_2d ** 2 + gamma / tau)))
        ...
        ru_k = opp2rgb(u_k, 0.6)
        psnr_graph[k] = psnr(np.clip(ru_k[:, 1:-1, 1:-1], 0, 255) / 255., u[:, 1:-1, 1:-1] / 255.)
        ssim_graph[k] = ssim(np.clip(u_k[:, 1:-1, 1:-1], 0, 255) / 255., u[:, 1:-1, 1:-1] / 255., channel_axis=0, data_range=2.0)
        mse_graph[k] = mse
        gradient_k = gradient_2(ru_k)
        gradient_graph[k] = np.log(np.linalg.norm(gradient_k))
        num_theta_k = dx_f2(ru_k[0]) * dy_f2(ru_k[1]) - dy_f2(ru_k[0]) * dx_f2(ru_k[1])
        den_theta_k = np.sqrt((dx_f2(ru_k[0]) ** 2 + dy_f2(ru_k[0]) ** 2) * (dx_f2(ru_k[1]) ** 2 + dy_f2(ru_k[1]) ** 2))
...
def plot_graphics(name_graphics_file):
    ...
    show_plot_cv2([matrix_graphics[1], matrix_graphics[2], matrix_graphics[3], matrix_graphics[4], matrix_graphics[5]],
                  ['PSNR', 'SSIM', 'mse', 'Gradient', 'Angle'], 'Graphics', cols_max=2, figsize=(14.2857*(3/2), 10*(3/2)),
...
def run_model():
    if sys.argv[1] == 'pdhg_opp_deb_gradient':
        # Change this line for the model to test
        graph_file_name, info_file_name, img_output = test_model(u, u_0, img_name, pdhg_opp_deb_gradient)
